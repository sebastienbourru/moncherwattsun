<?php
// ══════════════════════════════════════════════════════════
// generate-annuaire.php
// Script à lancer manuellement pour pré-charger les données
// Google Places des 15 villes en fichiers JSON.
//
// Usage : php generate-annuaire.php
// Ou via navigateur : https://moncherwattsun.fr/generate-annuaire.php
//
// Relancez ce script quand vous voulez rafraîchir les données
// (1 fois par mois par exemple).
// ══════════════════════════════════════════════════════════

// Sécurité : empêcher l'exécution par n'importe qui via navigateur
// Changez ce token par ce que vous voulez
$SECRET_TOKEN = 'wattsun2026';

// Si lancé via navigateur, vérifier le token
...
echo "═══════════════════════════════════════════\n";
echo "  Mon cher WattSun — Génération annuaire\n";
echo "═══════════════════════════════════════════\n\n";

foreach ($villes as $i => $ville) {
    $num = $i + 1;
    echo "[{$num}/15] {$ville['nom']} ({$ville['cp']})... ";
    ...
    if (!isset($data['results']) || empty($data['results'])) {
        echo "0 résultats\n";
        // Sauvegarder un fichier vide quand même
    ...
    foreach ($data['results'] as $place) {
        ...
        // ── Appel Place Details (téléphone, site web, horaires) ──
        if ($place_id) {
            ...
            // Pause 200ms entre chaque appel Details pour ne pas saturer le quota
            usleep(200000);
        }
    ...
    $nb = count($pros);
    $total_pros += $nb;
    echo "{$nb} pros trouvés ✓\n";

    // Pause 500ms entre chaque ville
    usleep(500000);
}
...
if (!empty($errors)) {
    ...
    echo "  Relancez le script pour réessayer.\n";
}
